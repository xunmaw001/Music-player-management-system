<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="会员类别" prop="huiyuanleibie">
          <el-input v-model="ruleForm.huiyuanleibie" 
              placeholder="会员类别" clearable  :readonly="ro.huiyuanleibie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="会员类别" prop="huiyuanleibie">
              <el-input v-model="ruleForm.huiyuanleibie" 
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `huiyuanleibie/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huiyuanleibieCrossAddOrUpdateFlag = false;
                  this.parent.search();
